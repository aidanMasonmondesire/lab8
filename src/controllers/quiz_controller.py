...
def create_quiz():
    ...
    # Initialize an instance of QuizService (Hint: service = QuizService())
    service = QuizService()
    ...
    # Use the service to create a quiz with the `data`
    #  and store the returned quiz ID in `quiz_id`
    new_quiz = service.create_quiz(data)
    quiz_id = new_quiz

    # INCOMPLETE: Return a JSON response with the quiz ID and a 201 status
    # Use jsonify to create a JSON response containing
    # `message` and `quiz_id`, with status code 201
...
def get_quiz(quiz_id):
    ...
    # If `quiz` exists, return it as a JSON response with status 200.
...
def submit_quiz(quiz_id):
    ...
    # Get the answers from the request using
    ...
    # Call `evaluate_quiz` with `quiz_id` and
    # `user_answers` and store the result in `score` and `message`
    ...
    # If `score` is None, return an error with status 404.
    # Otherwise, return `score` and `message` with status 200.
    if score is None:
        return jsonify({'error': 'evaulation unsuccessful'}), 404
    else:
