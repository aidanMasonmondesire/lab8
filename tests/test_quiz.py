...
def test_get_quiz(mock_get_quiz, client):
    ...
    mock_get_quiz.return_value = {'id': mock_quiz.id, 'title': mock_quiz.title, 'questions': mock_quiz.questions}
...
def test_submit_quiz(mock_evaluate_quiz, client):
    ...
    mock_evaluate_quiz.assert_called_once_with(1, user_answers['answers'])
